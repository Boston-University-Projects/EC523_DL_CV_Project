...
cd /projectnb/dl523/projects/RWD/EC523_DL_CV_Project/roboflow-ai/ScaledYOLOv4

# python train.py --batch-size 16 --img 640 640 --data $DATASET_DIR/data.yaml --cfg models/yolov4-p7.yaml --weights weights/yolov4-p7.pt --device 0 --name yolov4_p7_$TIMESTAMP --hyp 'data/hyp.finetune.yaml' --epochs 300

# If you want to do the distributed training (Assume you have 4 )
python -m torch.distributed.launch --nproc_per_node 4 train.py --batch-size 16 --img 640 640 --data $DATASET_DIR/data.yaml --cfg ./models/yolov4-p7.yaml --hyp data/hyp.finetune.yaml --weights weights/yolov4-p7.pt --sync-bn --device 0,1,2,3 --name yolov4_p7_$TIMESTAMP

python test.py --img 640 --conf 0.0 --batch 16 --device 0  --data $DATASET_DIR/data.yaml --weights runs/train/yolor_p6_$TIMESTAMP/weights/best_overall.pt  --task test --verbose  --save-txt --augment
